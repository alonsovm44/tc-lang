...
prompt_hash: bcc2edc4dd27990f
timestamp: 2026-06-07T22:33:39Z
...
This is the documentation for Tight-C's global, bulletproof hot reload functionality. Hot reload allows you to modify functions and structures (`strun`), as well as enum layouts in your application without restarting the main running application.
...
1. **Host Executable** (e.g., `hot_app.exe` on Windows):
   ...
   - Manages version tracking, loading/unloading DLL/SOs, and automatic file cleanup.

2. **Hot Library** (e.g., `hotlib_N.dll` on Windows / `hotlib_N.so` on Unix):
   - Contains all function bodies, structs (`strun`), and enums except for the main.
