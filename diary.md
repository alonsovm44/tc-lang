...
fn void main: {
  ...
  ->Point p = !foon() // !foon() otherwise comptime error
  defer free(p)
  printi(p.>x)
  printi(p.>y)
  // or this: free(p)
...
I added the whole picture to the 1.3.3 spec document. I deferred it to 1.3.3 because 1.3.2 is too big already. 
...
### extern removed
I removed extern blocks, i need to update the stdlib. For practically all modules i dont need to add inline C blocks with the library include stmt, since they're all covered with the stdlibs generated by the emitter.

## 23 June 2026
Tig now correctly calls the stdlib and links it properly. It can now be used anywhere in the system and this sets up the stage for the Tig installer and package manager. 
