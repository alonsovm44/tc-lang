...
Tight-C 1.2.0 introduces `_if` for chained conditionals, with `_` as the else wildcard:
...
- `_if(condition) { ... }` — else-if branch
- `_ { ... }` — final else branch (must be last)
- `_` is the wildcard pattern for "otherwise"
